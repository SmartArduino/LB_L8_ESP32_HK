...
static lv_obj_t *btn_qrObjSelect_A = NULL;
static lv_obj_t *btn_qrObjSelect_B = NULL;
...
static lv_style_t *styleBtnm_qrSelect_btnBg = NULL;
static lv_style_t *styleBtnm_qrSelect_btnRel = NULL;
static lv_style_t *styleBtnm_qrSelect_btnPre = NULL;
static lv_style_t *styleBtnm_qrSelect_btnTglRel = NULL;
static lv_style_t *styleBtnm_qrSelect_btnTglPre = NULL;
static lv_style_t *styleBtnm_qrSelect_btnIna = NULL;
static lv_style_t *styleBtnm_qrSelect_btnText = NULL;
...
static uint8_t qrCodeGenSelect = 0;
...
static void currentGui_elementClear(void){
	...
	if(btn_qrObjSelect_A){

		lv_obj_del(btn_qrObjSelect_A);
		btn_qrObjSelect_A = NULL;
	}
	if(btn_qrObjSelect_B){

		lv_obj_del(btn_qrObjSelect_B);
		btn_qrObjSelect_B = NULL;
	}
}
...
static lv_res_t funCb_btnActionClick_qrCodeSelect(lv_obj_t *btn){

	uint16_t btnFnum = lv_obj_get_free_num(btn);

	// printf("QR select:%d.\n", btSel);

	qrCodeGenSelect = btnFnum;
	...
	switch(btnFnum){
		...
		case 1:{if(NULL != btn_qrObjSelect_A)lv_btn_set_state(btn_qrObjSelect_A, LV_BTN_STATE_REL);}break;

		default:{if(NULL != btn_qrObjSelect_B)lv_btn_set_state(btn_qrObjSelect_B, LV_BTN_STATE_REL);}break;
	}
...
//														   (int)meshNetwork_connectReserve_IF_get());
	...
	switch(qrCodeGenSelect){
		...
		default:{
			...
			// sprintf(str_devMacBuff, "X-HM://002LK8JEXL8HT");
			homekitDevice_setupPayloadStr_get(str_devMacBuff);
...
//	printf("Qr code creat res:%d.\n", EncodeData("hellow, Lanbon!"));
	...
	externSocket_ex_disp_fill(0, 75, (devStatusDispMethod_landscapeIf_get())?(160):(240), (devStatusDispMethod_landscapeIf_get())?(240):(270), LV_COLOR_WHITE);
	...
	if(btn_qrObjSelect_A)lv_obj_refresh_style(btn_qrObjSelect_A);
	if(btn_qrObjSelect_B)lv_obj_refresh_style(btn_qrObjSelect_B);
}
...
//	vTaskDelay(1000 / portTICK_PERIOD_MS);
	...
	btn_qrObjSelect_A = lv_btn_create(obj_Parent, NULL);
	lv_btn_set_style(btn_qrObjSelect_A, LV_BTN_STYLE_REL, styleBtnm_qrSelect_btnRel);
	lv_btn_set_style(btn_qrObjSelect_A, LV_BTN_STYLE_PR, styleBtnm_qrSelect_btnPre);
	lv_btn_set_style(btn_qrObjSelect_A, LV_BTN_STYLE_TGL_REL, styleBtnm_qrSelect_btnTglRel);
	lv_btn_set_style(btn_qrObjSelect_A, LV_BTN_STYLE_TGL_PR, styleBtnm_qrSelect_btnTglPre);
	lv_btn_set_style(btn_qrObjSelect_A, LV_BTN_STYLE_INA, styleBtnm_qrSelect_btnIna);
	lv_obj_set_size(btn_qrObjSelect_A, 70, 40);
	lv_btn_set_toggle(btn_qrObjSelect_A, true);
	lv_obj_set_protect(btn_qrObjSelect_A, LV_PROTECT_POS);
	btn_qrObjSelect_B = lv_btn_create(obj_Parent, btn_qrObjSelect_A);
	if(devStatusDispMethod_landscapeIf_get()){

		lv_obj_set_pos(btn_qrObjSelect_A, 200, 110);
		lv_obj_align(btn_qrObjSelect_B, btn_qrObjSelect_A, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);

	}else{

		lv_obj_set_pos(btn_qrObjSelect_A, 40, 270);
		lv_obj_align(btn_qrObjSelect_B, btn_qrObjSelect_A, LV_ALIGN_OUT_RIGHT_MID, 20, 0);
	}
	lv_obj_set_free_num(btn_qrObjSelect_A, 0);
	lv_obj_set_free_num(btn_qrObjSelect_B, 1);
	lv_btn_set_action(btn_qrObjSelect_A, LV_BTN_ACTION_CLICK, funCb_btnActionClick_qrCodeSelect);
	lv_btn_set_action(btn_qrObjSelect_B, LV_BTN_ACTION_CLICK, funCb_btnActionClick_qrCodeSelect);
	lv_obj_t * lq_A = lv_label_create(btn_qrObjSelect_A, NULL);          /*Add a label to the button*/
	lv_obj_set_style(lq_A, styleBtnm_qrSelect_btnText);
	lv_label_set_text(lq_A, "Homekit");  
	lv_obj_t * lq_B = lv_label_create(btn_qrObjSelect_B, lq_A);          /*Add a label to the button*/
	lv_label_set_text(lq_B, "Smart\nliving");  

	lv_btn_set_state(btn_qrObjSelect_A, LV_BTN_STATE_TGL_REL);
	...
	if(ESP_OK != esp_wifi_sta_get_ap_info(&apInfo)){
		...
		btn_qrObjSelect_B = NULL;
		devStatusDispMethod_landscapeIf_get()?
			(lv_obj_set_pos(btn_qrObjSelect_A, 200, 135)):
			(lv_obj_set_pos(btn_qrObjSelect_A, 85, 270));
	}
...
static void lvGuiMenuWifiConfig_styleMemoryInitialization(void){
	...
	styleBtnm_qrSelect_btnBg 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtnm_qrSelect_btnRel 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtnm_qrSelect_btnPre 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtnm_qrSelect_btnTglRel 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtnm_qrSelect_btnTglPre 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtnm_qrSelect_btnIna 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtnm_qrSelect_btnText		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
}
...
static void lvGuiStyleInit_businessMenu_wifiConfig(void){
	...
	lv_style_copy(styleBtnm_qrSelect_btnBg, &lv_style_plain);
	styleBtnm_qrSelect_btnBg->body.opa = LV_OPA_70;
	styleBtnm_qrSelect_btnBg->body.radius = 4;
    styleBtnm_qrSelect_btnBg->body.main_color = lv_color_hsv_to_rgb(15, 11, 30);
    styleBtnm_qrSelect_btnBg->body.grad_color = lv_color_hsv_to_rgb(15, 11, 30);
    styleBtnm_qrSelect_btnBg->text.color = lv_color_hsv_to_rgb(15, 5, 95);

    lv_style_copy(styleBtnm_qrSelect_btnRel, &lv_style_pretty);
    styleBtnm_qrSelect_btnRel->body.main_color = lv_color_hsv_to_rgb(15, 10, 40);
    styleBtnm_qrSelect_btnRel->body.grad_color = lv_color_hsv_to_rgb(15, 10, 20);
    styleBtnm_qrSelect_btnRel->body.border.color = LV_COLOR_HEX3(0x111);
    styleBtnm_qrSelect_btnRel->body.border.width = 1;
    styleBtnm_qrSelect_btnRel->body.border.opa = LV_OPA_70;
    styleBtnm_qrSelect_btnRel->body.padding.hor = LV_DPI / 4;
    styleBtnm_qrSelect_btnRel->body.padding.ver = LV_DPI / 8;
    styleBtnm_qrSelect_btnRel->body.shadow.type = LV_SHADOW_BOTTOM;
    styleBtnm_qrSelect_btnRel->body.shadow.color = LV_COLOR_HEX3(0x111);
    styleBtnm_qrSelect_btnRel->body.shadow.width = LV_DPI / 30;
    styleBtnm_qrSelect_btnRel->text.color = LV_COLOR_HEX3(0xeee);

    lv_style_copy(styleBtnm_qrSelect_btnPre, styleBtnm_qrSelect_btnRel);
    styleBtnm_qrSelect_btnPre->body.main_color = LV_COLOR_MAKE(153, 217, 234);
    styleBtnm_qrSelect_btnPre->body.grad_color = LV_COLOR_MAKE(153, 217, 234);

    lv_style_copy(styleBtnm_qrSelect_btnTglRel, styleBtnm_qrSelect_btnRel);
    styleBtnm_qrSelect_btnTglRel->body.main_color = LV_COLOR_MAKE(153, 217, 234);
    styleBtnm_qrSelect_btnTglRel->body.grad_color = LV_COLOR_MAKE(153, 217, 234);
    styleBtnm_qrSelect_btnTglRel->body.shadow.width = LV_DPI / 40;
    styleBtnm_qrSelect_btnTglRel->text.color = LV_COLOR_MAKE(32, 32, 32);

    lv_style_copy(styleBtnm_qrSelect_btnTglPre, styleBtnm_qrSelect_btnRel);
    styleBtnm_qrSelect_btnTglPre->body.main_color = lv_color_hsv_to_rgb(15, 10, 10);
    styleBtnm_qrSelect_btnTglPre->body.grad_color = lv_color_hsv_to_rgb(15, 10, 30);
    styleBtnm_qrSelect_btnTglPre->body.shadow.width = LV_DPI / 30;
    styleBtnm_qrSelect_btnTglPre->text.color = LV_COLOR_HEX3(0xddd);

    lv_style_copy(styleBtnm_qrSelect_btnIna, styleBtnm_qrSelect_btnRel);
    styleBtnm_qrSelect_btnIna->body.main_color = lv_color_hsv_to_rgb(15, 10, 20);
    styleBtnm_qrSelect_btnIna->body.grad_color = lv_color_hsv_to_rgb(15, 10, 20);
    styleBtnm_qrSelect_btnIna->text.color = LV_COLOR_HEX3(0xaaa);
    styleBtnm_qrSelect_btnIna->body.shadow.width = 0;

	lv_style_copy(styleBtnm_qrSelect_btnText, &lv_style_plain);
	styleBtnm_qrSelect_btnText->text.font = &lv_font_consola_16;
	styleBtnm_qrSelect_btnText->text.color = LV_COLOR_WHITE;
}
