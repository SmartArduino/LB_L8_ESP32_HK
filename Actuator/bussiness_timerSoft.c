...
extern void htAccessoryDeviceDataPeriodUploadBussiness_loopReales(void);
...
static stt_timeZone devSystemTimeZone = {8, 0};
...
static void funCB_bussinessSoftTimer(void *timer){
	...
	if(loopTimer_1second.loopCounter < loopTimer_1second.loopPeriod)loopTimer_1second.loopCounter ++;
		...
		//homekit data period unpload
		htAccessoryDeviceDataPeriodUploadBussiness_loopReales();
			...
			if(ESP_OK == esp_wifi_sta_get_ap_info(&apInfo)){
				...
				if(!flg_stnp_running)sntp_applicationStart();
					...
					if(timeNow){
						...
						if(devSystemTimeZone.timeZone_H > 23){

							timeNow -= (3600UL * (long)(devSystemTimeZone.timeZone_H - 24) + 60UL * (long)devSystemTimeZone.timeZone_M); //时区负
						}
						else{
