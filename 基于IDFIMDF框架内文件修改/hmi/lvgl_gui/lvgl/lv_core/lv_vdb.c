...
#  if LV_VDB_ADR == 0
/*If the buffer address is not specified  simply allocate it*/
static uint32_t DEVL8_LV_VDB_SIZE_IN_BYTES = (uint32_t)(320 * 240 / 10);
static uint8_t vdb_buf1[(uint32_t)(320 * 240 / 10)];
static uint8_t vdb_buf2[(uint32_t)(320 * 240 / 10)];
...
void lv_flush_ready(void)
{
    ...
    memset(vdb_buf, 0x00, DEVL8_LV_VDB_SIZE_IN_BYTES);
#endif

#else
    if(vdb_state[0] == LV_VDB_STATE_FLUSH) {
#if LV_COLOR_SCREEN_TRANSP
        memset(vdb_buf[0], 0x00, DEVL8_LV_VDB_SIZE_IN_BYTES);
#endif
        vdb_state[0] = LV_VDB_STATE_FREE;
    }
    if(vdb_state[1] == LV_VDB_STATE_FLUSH) {
#if LV_COLOR_SCREEN_TRANSP
        memset(vdb_buf[1], 0x00, DEVL8_LV_VDB_SIZE_IN_BYTES);
#endif
