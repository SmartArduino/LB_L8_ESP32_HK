...
void lv_inv_area(const lv_area_t * area_p)
{
    ...
    scr_area.x1 = 0;
    scr_area.y1 = 0;
    scr_area.x2 = L8_DEV_SCREEN_SIZE_HOR - 1;
    scr_area.y2 = L8_DEV_SCREEN_SIZE_VER - 1;
...
static void lv_refr_area_with_vdb(const lv_area_t * area_p)
{
    ...
    lv_coord_t y2 = area_p->y2 >= L8_DEV_SCREEN_SIZE_VER ? y2 = L8_DEV_SCREEN_SIZE_VER - 1 : area_p->y2;

    int32_t max_row = (uint32_t)(L8_DEV_SCREEN_SIZE_VER * L8_DEV_SCREEN_SIZE_HOR / 10) / w;
